...
function buildWorkerAvailabilityForDate(nurses: Nurse[], dateKey: string): WorkerAvailabilityItem[] {
  return nurses
    ...
    .flatMap((nurse) => resolveVisibleShiftEntriesForDate(nurse, dateKey).map((entry) => ({
      ...
      nurseAddress: nurse.address || nurse.areas[0] || '',
      dateKey,
      start: entry.start,
      end: entry.end,
...
function mergeTooltipMaps(...maps: Record<string, string[]>[]): Record<string, string[]> {
  return maps.reduce<Record<string, string[]>>((acc, map) => {
    Object.entries(map).forEach(([key, labels]) => {
      acc[key] = dedupeLabels([...(acc[key] ?? []), ...labels]);
    });
    return acc;
  }, {});
}

function overlapsTime(left: { startMinutes: number; endMinutes: number }, right: { startMinutes: number; endMinutes: number }) {
  return left.startMinutes < right.endMinutes && right.startMinutes < left.endMinutes;
}

function findCrossBusinessNameConflict(
  visit: Pick<ScheduledVisit, 'slotId' | 'dateKey' | 'userName' | 'nurseName' | 'start' | 'end'>,
  otherBusinessEntries: OtherBusinessEntry[]
): string | null {
  for (const entry of otherBusinessEntries) {
    for (const other of entry.scheduledVisits) {
      if (other.dateKey !== visit.dateKey) continue;
      if (other.slotId === visit.slotId) continue;
      if (visit.userName.trim() && other.userName.trim() === visit.userName.trim()) {
        return `同日登録エラー: 利用者「${visit.userName}」は ${entry.businessName} にも予定があります（${other.start}-${other.end}）。`;
      }
      if ((visit.nurseName ?? '').trim() && (other.nurseName ?? '').trim() === (visit.nurseName ?? '').trim()) {
        return `同日登録エラー: 看護師「${visit.nurseName}」は ${entry.businessName} にも予定があります（${other.start}-${other.end}）。`;
      }
    }
  }
  return null;
...
function buildOtherBusinessEntries(
  businesses: BusinessWorkspace[],
  activeBusinessId: string,
  snapshots: Record<string, BusinessSnapshot>
): OtherBusinessEntry[] {
  return businesses
    .filter((business) => business.id !== activeBusinessId)
    .map((business) => {
      const snapshot = snapshots[business.id];
      ...
      return {
        ...
        scheduledVisits: snapshot.scheduledVisits ?? []
      } satisfies OtherBusinessEntry;
    })
    .filter((item): item is OtherBusinessEntry => Boolean(item));
}
...
export default function App() {
  ...
  const otherBusinessEntries = useMemo(() => buildOtherBusinessEntries(
    businesses,
    activeBusinessId,
    loadFromStorage<Record<string, BusinessSnapshot>>(BUSINESS_SNAPSHOT_KEY, {})
  ), [businesses, activeBusinessId, interactionVersion]);
  ...
  const confirmCandidateAsFixed = async (slotId: string, targetDateKey?: string) => {
    ...
    const crossBusinessConflict = findCrossBusinessNameConflict(scheduled, otherBusinessEntries);
    if (crossBusinessConflict) {
      throw new Error(crossBusinessConflict);
    }
  ...
  const handleUpdateScheduled = async (visit: ScheduledVisit) => {
    ...
    const crossBusinessConflict = findCrossBusinessNameConflict(nextVisit, otherBusinessEntries);
    if (crossBusinessConflict) {
  ...
  const crossBusinessScheduleWarnings = useMemo(() => {
    const messages = new Map<string, ConflictWarning>();
    scheduledVisits.forEach((visit) => {
      otherBusinessEntries.forEach((entry) => {
        entry.scheduledVisits
          .filter((other) => other.dateKey === visit.dateKey)
          .forEach((other) => {
            if (visit.userName.trim() && other.userName.trim() === visit.userName.trim()) {
              const key = `user-${visit.dateKey}-${visit.userName}-${entry.businessId}-${other.slotId}`;
              messages.set(key, {
                type: 'cross-business-user',
                dateKey: visit.dateKey,
                slotIds: [visit.slotId],
                message: `${visit.dateKey}: 利用者「${visit.userName}」が ${entry.businessName} にも存在し、同日に予定があります (${visit.start}-${visit.end} / ${other.start}-${other.end})`
              });
            }
            if ((visit.nurseName ?? '').trim() && (other.nurseName ?? '').trim() === (visit.nurseName ?? '').trim()) {
              const overlapNote = overlapsTime(visit, other) ? '時間重複あり' : '同日登録あり';
              const key = `nurse-${visit.dateKey}-${visit.nurseName}-${entry.businessId}-${other.slotId}`;
              messages.set(key, {
                type: 'cross-business-nurse',
                dateKey: visit.dateKey,
                slotIds: [visit.slotId],
                message: `${visit.dateKey}: 看護師「${visit.nurseName}」が ${entry.businessName} にも登録されています (${overlapNote} / ${visit.start}-${visit.end} / ${other.start}-${other.end})`
              });
            }
          });
      });
    });
    return Array.from(messages.values()).sort((a, b) => a.dateKey.localeCompare(b.dateKey) || a.message.localeCompare(b.message, 'ja'));
  }, [scheduledVisits, otherBusinessEntries]);

  const warnings = useMemo(() => [...buildConflictWarnings(scheduledVisits), ...crossBusinessScheduleWarnings], [scheduledVisits, crossBusinessScheduleWarnings]);
  ...
  const crossBusinessUserTooltipMap = useMemo(() => users.reduce<Record<string, string[]>>((acc, user) => {
    const labels = otherBusinessEntries.flatMap((entry) => entry.users
      .filter((candidate) => candidate.利用者名.trim() && candidate.利用者名.trim() === user.利用者名.trim())
      .map((candidate) => `${candidate.利用者名}（${entry.businessName} / ${candidate.居住地 || '住所未設定'}）`));
    if (labels.length) acc[user.id] = dedupeLabels(labels);
    return acc;
  }, {}), [users, otherBusinessEntries]);

  const duplicateUserIdsAll = useMemo(() => new Set([
    ...duplicateUserIds,
    ...Object.keys(crossBusinessUserTooltipMap)
  ]), [duplicateUserIds, crossBusinessUserTooltipMap]);

  const duplicateUserTooltipMapAll = useMemo(() => mergeTooltipMaps(duplicateUserTooltipMap, crossBusinessUserTooltipMap), [duplicateUserTooltipMap, crossBusinessUserTooltipMap]);

  const crossBusinessUserAlerts = useMemo(() => users
    .filter((user) => (crossBusinessUserTooltipMap[user.id] ?? []).length > 0)
    .map((user) => ({
      id: user.id,
      name: user.利用者名,
      subtitle: user.居住地 || '住所未設定',
      labels: crossBusinessUserTooltipMap[user.id] ?? []
    }))
  ...
  const duplicateNurseTooltipMap = useMemo(() => nurses.reduce<Record<string, string[]>>((acc, nurse) => {
    ...
    if (labels.length) acc[nurse.id] = dedupeLabels(labels);
    return acc;
  ...
  return (
    <div className="app-shell">
      ...
      {(crossBusinessUserAlerts.length > 0 || crossBusinessNurseAlerts.length > 0) && (
        <section className="card panel duplicate-user-alert-panel cross-business-alert-panel">
          <div className="split-line unassigned-alert-header">
            <div>
              <h2>事業所横断の名前重複アラート</h2>
              <p className="helper-text">他の事業所にも同じ名前があるため、赤枠で注意表示しています。同日予定がある場合は重複警告にも反映されます。</p>
            </div>
            <div className="alert-count-badge duplicate-count-badge">{crossBusinessUserAlerts.length + crossBusinessNurseAlerts.length}件</div>
          </div>
          <div className="cross-business-alert-grid">
            {crossBusinessUserAlerts.length > 0 && (
              <article className="mini-card duplicate-user-card">
                <strong>利用者名</strong>
                <div className="duplicate-user-chip-list">
                  {crossBusinessUserAlerts.map((row) => (
                    <button
                      key={row.id}
                      type="button"
                      className="duplicate-user-chip"
                      onClick={() => handleOpenUserEditor(row.id)}
                    >
                      <strong>{row.name}</strong>
                      <span>{row.subtitle}</span>
                      <span>{row.labels.join(' / ')}</span>
                    </button>
                  ))}
                </div>
              </article>
            )}
            {crossBusinessNurseAlerts.length > 0 && (
              <article className="mini-card duplicate-user-card">
                <strong>看護師名</strong>
                <div className="duplicate-user-chip-list">
                  {crossBusinessNurseAlerts.map((row) => (
                    <article key={row.id} className="duplicate-user-chip">
                      <strong>{row.name}</strong>
                      <span>{row.subtitle}</span>
                      <span>{row.labels.join(' / ')}</span>
                    </article>
      ...
      <main className="scheduler-main">
        <CalendarView
          ...
          duplicateUserIds={[...duplicateUserIdsAll]}
          duplicateUserTooltips={duplicateUserTooltipMapAll}
          duplicateNurseIds={[...duplicateNurseIds]}
          duplicateNurseTooltips={duplicateNurseTooltipMap}
          viewMode={viewMode}
          periodLabel={formatMonthLabel(currentDate)}
          selectedNurseName={selectedNurseLabel}
        />
